...
As implemented in Pyrus, the accuracy of the Pedersen method did not replicate the published results. This is more likely an issue with the code itself rather than the method. The consequence is that other corresponding states methods have been examined, including that of Lucas. The Lucas method is effective, and lends weight to the assertion that it is the Pyrus-specific implementation of the Pedersen method that is flawed rather than the corresponding states technique itself.
