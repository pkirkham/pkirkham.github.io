...
<div class="notice-info">In preparation for sharing the Java code, much of unit conversion logic using the JScience `Amount` class was stripped out to speed up execution in Java. For example, rather than using `double t_degr = Amount.valueOf(temperaure, FAHRENHEIT).doubleValue(RANKINE);`, it has been replaced with `double t_degr = temperature + 459.67; // degF to degR`. This removes a lot of the overhead of JScience `Amount` class. Usually I am fastidious about units, as explained in my post on [use of generics for physical units]({{ site.url }}/pyrus/java-generics-units-strong-typing/), but in this case the low-level code can't justify the overhead for simply converting between Fahrenheit and Rankine. Benchmark results with the code as shown in this blog post are about 2&times; to 3&times; slower.</div>
