...
<div class="notice-info">In preparation for sharing the Java code, much of unit conversion logic using the JScience <code>Amount</code> class was stripped out to speed up execution in Java. For example, rather than using <code>double t_degr = Amount.valueOf(temperaure, FAHRENHEIT).doubleValue(RANKINE);</code>, it has been replaced with <code>double t_degr = temperature + 459.67; // degF to degR</code>. This removes a lot of the overhead of JScience <code>Amount</code> class. Usually I am fastidious about units, as explained in my post on <a href="{{ site.url }}/pyrus/java-generics-units-strong-typing/">use of generics for physical units</a>, but in this case the low-level code can't justify the overhead for simply converting between Fahrenheit and Rankine. Benchmark results with the code as shown in this blog post are about 2&times; to 3&times; slower.</div>
